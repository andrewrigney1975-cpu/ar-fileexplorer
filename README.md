...
### Backup & restore (VSS file-level)
Command palette → **"Backup..."** opens the backup manager: define jobs, each with a source (a whole volume like `C:\` or any folder), a destination folder (external drive, network share, another volume), a full-backup interval (every X days) and a differential interval (every Y days in between), and how many full sets to keep.

- **How a run works:** the job runs in an elevated helper instance of the app (re-launched with `--backup-run`; UAC prompts once). When "Use Volume Shadow Copy" is on, it scripts `diskshadow.exe` (built into Windows, no library) to snapshot the source volume, then `robocopy` mirrors the snapshot into a dated set folder with `/COPYALL` (ACLs, owner, auditing, alternate data streams), reparse points copied as links, and junctions skipped. Open/locked files are captured from the snapshot. The snapshot is deleted afterwards.
- **Full vs differential:** a **Full** set is a complete standalone copy. A **Differential** set is hard-link-seeded from the latest full — unchanged files are hard links that cost no extra disk space (the `rsync --link-dest` model), and only changed/new files are stored for real. Changed files are unlinked before the copy so writing them never touches the base full set. The manager auto-picks the mode (full if none exists or the newest full is older than X days, otherwise differential); you can also force **Run full now** / **Run differential now**.
- **Scheduling:** tick "Scheduled" on a job to register it with the app's `ScheduleService` (fires on the differential interval; each run resolves full-vs-differential itself). Retention prunes the oldest fulls (and their orphaned differentials) beyond the keep count after a successful run.
- **Restore:** pick a completed set and a destination folder; `robocopy` copies the tree back additively (no `/MIR`, so it never deletes anything at the destination). Every set folder is a plain directory tree you can also just open in Explorer and copy from by hand.
- **Scope (deliberate v1):** file-level only — not a bootable disk image, not raw sectors, no partition/format cloning, no bare-metal restore. The pure-logic core (`BackupService` set bookkeeping, `BackupRunner` full/differential run) carries zero WinUI dependency and is covered by end-to-end tests (real `robocopy`, real hard-link differential on temp folders).
