...
### Navigation & layout
- Every icon-only action (Undo, Cut/Copy/Paste — with the standard `Ctrl+X`/`Ctrl+C`/`Ctrl+V` accelerators — Delete, New folder, the three Disk tools below, the four view-mode toggles, and the Terminal toggle) lives in a single VS Code–style left **activity bar** (52px), rather than a top toolbar; Sync tasks and File operations get their own activity-bar buttons that open a flyout list instead of navigating anywhere
- The activity bar's Delete button deletes the current selection to the Recycle Bin (same as `Del`); right-clicking it instead empties **every drive's** Recycle Bin at once (confirmation required) via the shell's own empty-bin call
- A pinned, non-closable **Home** tab always occupies the first Workspace slot: drive picker (the same grid the Disk Space Analyser uses) as its left pane, the system drive's root as its right — rebuilt fresh every launch rather than restored from session state
...
- Cut / Copy / Paste from the activity bar or context menu, with a shared clipboard between panes
...
- An activity-bar flyout (next to File operations) lists the sync tasks whose source folder is visible in the current Workspace's Left or Right pane, with a trash icon (and confirmation) to delete a task; running a task enqueues it into the same File operations queue/list as any copy or move, with live progress
...
- Scripts run off the UI thread with a 180-second timeout guard; `deleteItem` defaults to the Recycle Bin; script-driven file changes are **not** tracked by Undo, and there's no sandboxing beyond the timeout (the app already ships a full terminal, so a script has no more reach than the user already does)
...
- 0-5 star ratings (right-click → "Rate", or "Clear rating"): an item's **effective** rating is its own explicit rating if set, else — for a folder — the average of its direct children's effective ratings, else its parent's effective rating inherited downward; only an explicit rating is shown at full opacity, an inherited/averaged one at reduced opacity so the two are visually distinguishable. Shown as a star-rating column in Details view and as a small overlay badge on each Gallery tile; Search Everywhere has its own minimum-rating filter dropdown
- Duplicate file finder (size-then-hash scan) with a review-and-delete dialog; deletion runs off the UI thread with a live "Deleting N of M..." progress dialog, so a large batch (e.g. cleaning up a slow removable drive) never looks like a hang. Also reachable from the right-click menu — on a folder (scoped to that folder) or on empty pane space (scoped to the pane's current folder) — not just F4/command palette. When the target location is covered by the Search Everywhere index, the finder identifies duplicates from the index's stored file sizes and MD5 hashes instead of re-walking and re-reading the tree — a pre-scan dialog says so and offers a **Force disk-based duplicate identification** checkbox to override; files the index has no hash for yet are still checked on disk
...
- Search Everywhere (`F9`, or the command palette): instant substring search across a persistent, local SQLite index of whichever folders/drives you've added under Control Centre → Search Index — separate from the per-pane recursive search above, which always walks the live filesystem instead. Indexing is opt-in per folder (nothing is scanned until you add one — no "index everything" default) and stays current via a recursive `FileSystemWatcher` per indexed root plus a full rescan every 24h as a backstop. Deliberately not built on the USN journal (would need admin rights, which this unpackaged app never requests) or the OS's own Windows Search indexer (only covers the user profile/Libraries by default, so a data drive would silently return nothing). Substring matching is backed by an FTS5 trigram index (an indexed `MATCH`, not an `O(rows)` `LIKE '%x%'` scan) — queries of three or more characters use it, shorter ones fall back to the plain scan; the trigram index is populated once in the background on first launch after the feature lands and kept current by triggers after that. The SQL match/substring query itself is capped at 10,000 raw candidate rows (up from 2,000), ordered by name length before that cap is applied, before any app-side fuzzy scoring happens — without that ordering, a common substring matching more rows than the cap (confirmed: a 12-character folder name lost to 2,088 total matches, mostly 80-character filenames merely containing the same substring) could have the exact match a user wants silently dropped by an arbitrary, relevance-blind SQL-level truncation, regardless of how well it would have scored once it reached the app's own ranking. Results are ranked with the same typo-tolerant `FuzzyMatcher` the per-pane search uses (now with an optional exact-case mode — see below); double-click or Enter opens the result in a brand-new "Search Results" Workspace tab (rather than navigating whatever pane was last active), so existing workspaces are never disturbed. **Search From Here** (`Alt+F9`, or the command palette): the same dialog and index, restricted to the active pane's current folder and everything nested under it — for when you know roughly where something is and don't want results from the rest of the index. Two checkboxes next to the rating filter refine ranking rather than filtering results out: **Case sensitive** requires exact-case matches (both the substring and subsequence fuzzy-match passes switch from ordinal-ignore-case to ordinal comparison); **Prioritise folders** sorts every matched folder ahead of every matched file, still ranked by match quality within each group. Each indexed row stores the file's size in bytes; its MD5 hash (filled in by a low-priority background pass, not the walk — only for files whose exact byte size collides with another indexed file, since a size-unique file can't have a duplicate) lives in a separate `FileHashes` table, keyed by directory + filename rather than as a column on the indexed row itself. That's what lets a hash survive a re-index: "Rebuild now", a single-root re-index, and "Index From Here..." all delete and recreate their `Entries` rows, but never touch `FileHashes`, so a file whose size/modified-time haven't changed keeps its previously-computed hash (very expensive to redo on a large index) instead of the backfill starting over from scratch — only a file that's actually changed since its hash was computed gets re-hashed. Control Centre → Search Index shows each indexed location's own entry count (with its own re-index button, for refreshing just one location), the index's total size on disk, and a progress ring with a live-updating count while a scan is running. Re-indexing a single already-indexed root first deletes just that root's existing rows (one indexed delete, not per-row) so the walk that follows is all fresh inserts rather than SQLite `ON CONFLICT` updates — updating ~1.2M already-indexed rows measured 20x slower than inserting the same rows fresh, and the walker has no way to report progress while it's blocked waiting for a backlogged writer to catch up, so re-indexing a large already-indexed root used to look completely frozen for however long that took. Re-indexing no longer loses that root's hashes — they live in the separate `FileHashes` table (see above) and are simply reattached once the walk re-inserts the same files. Whenever the walker *is* genuinely paused on a write backlog (a slow drive, not just a re-index), the status line now says so explicitly ("Writer catching up (N batches queued)...") instead of looking indistinguishable from a hang. The walk never descends into a reparse point (a directory junction or symlink) — the link itself still gets a row (findable by name), but its target's contents don't get walked a second time, which matters when one configured root's tree happens to contain a junction into another configured root (a common StableBit DrivePool-style layout): without this, every file under the junction was indexed twice, under two different paths, for no benefit. Rows are batched 50,000 to a transaction before being posted to the single writer thread (up from 2,000 — fewer, larger commits amortize each commit's fixed overhead across far more rows), and the writer checkpoints the WAL back into the main database file every 2 minutes while a scan is running rather than only once the whole rebuild finishes, so a long-running scan's WAL file can't grow unboundedly (confirmed climbing past 400MB, uncapped, before this). The MD5 backfill itself can be paused for 6 hours at a time ("Pause hashing (6h)" / "Resume hashing" toggle) if it's competing for disk I/O with something else — the pause is file-backed, so it survives an app restart. On top of that manual pause, a daily local-time hashing window (Control Centre → Search Index, defaulting to **22:00–04:00**, every day) restricts the backfill to run only inside it — indexing, watching, and search are unaffected, only the hashing pass idles outside the window; an end time earlier than the start wraps past midnight, and the restriction can be turned off entirely. A table beneath it breaks hashed-vs-remaining file counts (and remaining bytes) down by size bucket (<1MB, 1-10MB, 10-100MB, 100MB-1GB, >1GB), refreshed on its own 120-second background timer so it can't add to the 1-second status poll's UI-thread cost. The walk buffers rows and commits them in short batched transactions rather than holding one transaction open for the whole run, indexes each root independently so one unreadable drive can't abort the rest (or skip their stale-row cleanup), and never indexes the app's own state/log directory — which, since a drive root is usually an indexed root, would otherwise have every index write trip a watcher event that triggered another index write. Every filesystem call carries a watchdog timeout (skip-and-continue, logged) - plain blocking Win32 APIs have no cancellation support, so an unresponsive drive (spun down, a failing USB/SATA bridge) could otherwise stall the entire scan forever with no way to recover short of restarting the app. Every SQLite connection sets a 30s `busy_timeout` so a scan's writes and the watcher's debounced writes don't fail each other outright on contention; the Search Index panel also polls once a second while it's the visible section (diffed against what's already shown, so it doesn't flicker), rather than depending solely on a change event to know a scan finished. While a scan is running, the status line shows the folder currently being walked; the MD5 backfill gets its own distinct "Hashing..." status (with a running count for the current pass) instead of one ambiguous "working" indicator. Right-click any folder → "Exclude from indexing and hashing" to keep it (and everything nested under it) out of the walk, the watcher, and the hash backfill going forward, and delete any rows already indexed under it immediately; Control Centre → Search Index → "Show excluded parent paths" lists every exclusion with a one-click way to remove it. "Rebuild now" always wipes the whole index before re-walking, so an excluded folder can never resurface as a stale row. Right-click any folder → **"Index From Here..."** to rescan just that folder and everything nested under it — same delete-then-insert approach as a single-root re-index, scoped to a subfolder instead of a whole root: deletes every existing row at or under that folder (one indexed delete by path) and walks it fresh, without touching the rest of the index. New rows are tagged with whichever configured root actually contains the folder, so they stay grouped with that root the same as an ordinary scan's rows; a folder outside every configured root still gets indexed (tagged with its own path as an ad-hoc root) but won't be watched, won't get the 24h backstop rescan, and disappears the next time "Rebuild now" runs, since it's never added to the roots list
...
- Last-resort preview: anything no other handler can render (binaries, unrecognized extensions) shows a hex + ASCII dump of the first 8KB instead of just a bare file icon
...
- **Add and delete while unlocked**: drag-drop or paste real files/folders into an unlocked encrypted folder to encrypt them straight into the container, use **New folder** from the empty-space menu, and delete items normally (permanently — there's no Recycle Bin inside a container, same as a remote connection). Every add/delete rewrites the whole container to a verified temp file and atomically swaps it in, so it's always valid on disk after each individual operation, never left mid-batch. Copying files back *out* of an encrypted folder into another location isn't supported yet — open the file, or use Decrypt Folder to remove encryption entirely. **Compact fragmented space** (same menu) is a manual, explicit rewrite-in-place — normally a no-op, since every add/delete already rebuilds without leftover bytes, but it's there as a safety valve
...
### Other
- Built-in terminal drawer, opens at the active pane's folder; can be turned off from Control Centre → Preferences (hides its activity-bar toggle and closes it if open)
